...
app.post("/equipment/:DocID", function(req, res) {
    let stat = req.body.update_tinh_trang_thiet_bi_1;
    if(stat === "Khả Dụng"){
        stat = "available";
    }
    else if(stat === "Không Khả Dụng"){
        stat = "unavailable"
    }
    else {
        stat = "maintenance"
    ...
    console.log(stat);
    let equipmentName = req.body.equipment_name;
    FindandChange(equipmentName, stat);
    // Assuming you want to redirect back to the equipment page after updating
